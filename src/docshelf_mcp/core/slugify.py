...
def slugify(text: str, max_len: int = 60) -> str:
    ...
    Returns:
        The slug, or ``""`` when the input slugifies to nothing (pure
        punctuation or whitespace). An empty return lets each call site apply
        its own meaningful fallback (e.g. ``slugify(title) or "document"``);
        callers that build a path from the result must guard against ``""``.
